...
scheduleReceiver.addToRolePolicy(new iam.PolicyStatement({
  actions: ["dynamodb:GetItem", "dynamodb:BatchGetItem", "dynamodb:PutItem", "dynamodb:UpdateItem", "dynamodb:Query"],
  resources: [tableArn, `${tableArn}/index/*`],
}));
scheduleReceiver.addToRolePolicy(new iam.PolicyStatement({ actions: ["ssm:GetParameters"],
  resources: scheduleParameters.map(p => stack.formatArn({ service: "ssm", resource: "parameter", resourceName: p.slice(1) })),
}));
