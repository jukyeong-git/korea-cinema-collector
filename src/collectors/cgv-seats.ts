...
import { CGV_USER_AGENT, CgvHttpError } from "./cgv-api";

export const PREFERRED_ROWS = "GHIJKLMNO";
export function isPreferredSeat(row: string, number: number): boolean {
  return row.length === 1 && PREFERRED_ROWS.includes(row)
    && ((number >= 7 && number <= 14) || (number >= 16 && number <= 29) || (number >= 31 && number <= 38));
}
...
export function parseSeatResponse(value: unknown, candidate: SeatCandidate): SeatSnapshot {
  ...
  if (seen.size !== 624 || target.size !== 270) throw Error(`Unexpected Yongsan seat layout: ${seen.size}/${target.size}`);
